...
extern "C" void input_function(void *pvParameters) 
{
    ...
    while (1) 
    {
        ...
        else if(input_function_stage > 20 && input_function_stage  <= 30)
        {
            voltage = 3 + 0.17*(input_function_stage - 10);
            pwm_value = voltage*8.33;
            pwm_timer_4.pwmWrite(pwm_value, TIM_OC1);
            gpio_clear(GPIOB, GPIO8);
            pwm_timer_4.pwmWrite(pwm_value, TIM_OC2);
            gpio_clear(GPIOB, GPIO9);
        }
        else if(input_function_stage > 30 && input_function_stage  <= 50)
        {
            voltage = 5;
            pwm_value = voltage*8.33;
            pwm_timer_4.pwmWrite(pwm_value, TIM_OC1);
        ...
        else if(input_function_stage > 50 && input_function_stage  <= 60)
        {
        ...
        else if(input_function_stage > 60 && input_function_stage  <= 70)
        {
            voltage = 3;
            pwm_value = voltage*8.33;
            pwm_timer_4.pwmWrite(pwm_value, TIM_OC1);
        ...
        else if(input_function_stage > 70 && input_function_stage  <= 80)
        {
            voltage = 0;
            pwm_value = voltage*8.33;
            pwm_timer_4.pwmWrite(pwm_value, TIM_OC1);
        ...
        else if(input_function_stage > 80 && input_function_stage <= 90)
        {
            voltage = 0;
            pwm_value = voltage*8.33;
            pwm_timer_4.pwmWrite(pwm_value, TIM_OC1);
        ...
        else if(input_function_stage > 90 && input_function_stage  <= 100)
        {
            voltage = -3;
            pwm_value = 100 + voltage*8.33;
            pwm_timer_4.pwmWrite(pwm_value, TIM_OC1);
            gpio_set(GPIOB, GPIO8);
            pwm_timer_4.pwmWrite(pwm_value, TIM_OC2);
            gpio_set(GPIOB, GPIO9);
        }
        else if(input_function_stage > 100 && input_function_stage  <= 110)
        {
            voltage = -(3 + 0.17*(input_function_stage - 100));
            pwm_value = 100 + voltage*8.33;
            pwm_timer_4.pwmWrite(pwm_value, TIM_OC1);
            gpio_set(GPIOB, GPIO8);
            pwm_timer_4.pwmWrite(pwm_value, TIM_OC2);
        ...
        else if(input_function_stage > 110 && input_function_stage  <= 130)
        {
            voltage = -5;
            pwm_value = 100 + voltage*8.33;
            pwm_timer_4.pwmWrite(pwm_value, TIM_OC1);
        ...
        else if(input_function_stage > 130 && input_function_stage  <= 140)
        {
        ...
        else if(input_function_stage > 140 && input_function_stage  <= 150)
        {
            voltage = -3;
            pwm_value = 100 + voltage*8.33;
            pwm_timer_4.pwmWrite(pwm_value, TIM_OC1);
        ...
        else if(input_function_stage > 150 && input_function_stage  <= 160)
        {
            voltage = 0;
            pwm_value = 0;
            pwm_timer_4.pwmWrite(pwm_value, TIM_OC1);
