...
//ADPI_Controller motor_controller_two(45, 12.5, 3200);
//P_Controller motor_controller_one(45, 3200);
ISubject system_manager;

controller joint_one(0, GPIOB, GPIO6, 
                        &adc_port_a, &motor_controller_one,
                        &pwm_timer_4, RCC_GPIOB);
/*
controller joint_two(1, GPIOB, GPIO7, 
                        &adc_port_a, &motor_controller_two,
                        &pwm_timer_4, RCC_GPIOB);
*/
/*
 controller joint_three(2, GPIOB, GPIO8, 
                        &adc_port_a, &motor_controller_three,
                        &pwm_timer_4, RCC_GPIOB);
*/
...
/*
extern "C" void Joint_Two(void *pvParameters) 
{
    (void)pvParameters;

    while(true) 
    {
        serial_interface.usartSend_char("Joint_two Potent[k]: ");
        serial_interface.usartSend_integer(joint_two.log_sensor_value());
...
/*
extern "C" void Joint_Three(void *pvParameters) 
{
    (void)pvParameters;

    while(true) 
    {
        serial_interface.usartSend_char("Joint_three Potent[k]: ");
        serial_interface.usartSend_integer(joint_two.log_sensor_value());
        vTaskDelay(pdMS_TO_TICKS(125));
    }
}
...
int main(void)
{
    ...
    //joint_two.time_period = SYSTEM_TICK_MS(10);
    //joint_three.time_period = SYSTEM_TICK_MS(10);

    system_manager.Attach(&joint_one);
    //system_manager.Attach(&joint_two);
    //system_manager.Attach(&joint_three);

    //xTaskCreate(Joint_One, "Joint_One", configMINIMAL_STACK_SIZE, nullptr, configMAX_PRIORITIES - 1, nullptr);
    //xTaskCreate(Joint_Two, "Joint_Two", configMINIMAL_STACK_SIZE, nullptr, configMAX_PRIORITIES - 2, nullptr);
    //xTaskCreate(Joint_Three, "Joint_Three", configMINIMAL_STACK_SIZE, nullptr, configMAX_PRIORITIES - 3, nullptr);
    ...
    { 
        ...
        if(g_counter_millis - log_time > SYSTEM_TICK_MS(125))
        {
