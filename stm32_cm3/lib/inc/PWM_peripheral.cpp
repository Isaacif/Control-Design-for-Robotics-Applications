...
void PWM_peripheral::PWM_initialization(uint32_t T_REG, uint32_t TC_REG, rcc_periph_clken RCC_TSelect)
{
    timer_counter_register = TC_REG, timer_register = T_REG, RCC_TIMER_SELECT = RCC_TSelect;
                               ...
                               uint32_t GPIO_Pin_Select, rcc_periph_clken RCC_GPort_Select)
{
    ...
    gpio_mode_setup(GPIO_Port_Select, GPIO_MODE_AF,
                    GPIO_PUPD_NONE, GPIO_Pin_Select);
    gpio_set_af(GPIO_Port_Select, GPIO_AF2, GPIO_Pin_Select);

    //gpio_set_output_options(GPIO_Port_Select, GPIO_OTYPE_PP,
                    //GPIO_OSPEED_50MHZ, GPIO_Pin_Select);
...
void PWM_peripheral::pwmWrite(uint16_t duty_cycle, tim_oc_id timer_select)
{
    uint32_t busy_time = (duty_cycle*COUNT_UP_TO)/100;
    //uint32_t busy_time = duty_cycle;
    timer_disable_oc_output(timer_register, timer_select);
    timer_set_oc_mode(timer_register, timer_select, TIM_OCM_PWM1);
    timer_set_oc_value(timer_register, timer_select, busy_time); // 50% duty cycle
